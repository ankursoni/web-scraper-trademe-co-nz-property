...
from .property_search import (DEFAULT_CITY, DEFAULT_DOMAIN_URL,
                              DEFAULT_HIT_DELAY_SECONDS, PropertySearch,
                              property_search_fetch)


def search_without_detail(domain_url=DEFAULT_DOMAIN_URL,
                          city=DEFAULT_CITY,
...
def search_with_detail(domain_url=DEFAULT_DOMAIN_URL,
                       city=DEFAULT_CITY,
                       total_pages=1) -> list[PropertyDetail]:
...
def main():
    ...
    arg_do_search_with_detail = distutils.util.strtobool(sys.argv[3]) if len(sys.argv) > 3 \
        else False
    ...
    search_result = search_with_detail(city=arg_city, total_pages=arg_total_pages) \
