...
def property_search_fetch(domain_url=DEFAULT_DOMAIN_URL,
                          city=DEFAULT_CITY,
                          total_pages=1) -> list[PropertySearch]:
    ...
    for page_number in range(1, total_pages+1):
        ...
        property_listing_cards = soup.find_all(
        ...
        for plc in property_listing_cards:
            ...
            tag = plc.find(
                'a', class_='tm-property-premium-listing-card__link')
            if tag:
                property_search.link = urllib.parse.urljoin(domain_url, urllib.parse.urlparse(
                    str.strip(tag.attrs['href'])).path)
            ...
            tag = plc.find(
                'tm-property-search-card-price-attribute')
            if tag:
...
def _parse_property_search_card_attribute(plc_icons, property_search):
    ...
    tag = plc_icons.find(
        'tg-icon', attrs={'name': 'vehicle-car-front'})
    if tag:
        property_search.number_of_parking_lots = str.strip(
            tag.nextSibling.text)

    tag = plc_icons.find('tg-icon', attrs={'name': 'armchair'})
    if tag:
        property_search.number_of_living_areas = str.strip(
            tag.nextSibling.text)

    tag = plc_icons.find(
        'tg-icon', attrs={'name': 'house-blueprint'})
    if tag:
        property_search.floor_area_sqm = str.strip(tag.nextSibling.text)

    tag = plc_icons.find(
        'tg-icon', attrs={'name': 'house-land-area'})
    if tag:
